...
These are mechanical checks that must be applied exhaustively across all changed files. Do not sample — check every instance.

- **Doc headers** — every public type, function, method, and interface must have a doc comment. Flag every instance that is missing one, do not summarize (e.g., "17 of 20 structs missing doc comments" is not acceptable — list each one).
