...
1. **If an editor agent is configured** (`[author.dispatch].editor`), apply the dispatch threshold:
   ...
   **Severity mapping:** Suggestion severity uses the same (H)/(M)/(L) priority labels from V-1 review findings. Medium+ means any suggestion tagged (H) or (M). Low means (L) or untagged.
