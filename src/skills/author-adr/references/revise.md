...
- **If not found (first round):** Append the HTML comment and Q&A entries
  after any existing content in `## Comments` (e.g., after a Draft Worksheet
  if one is present, or directly under the heading if Comments is empty):
...
If no Draft Worksheet exists, proceed with triage using only the review output and ADR content — the worksheet is optional.
