...
The template for QA plan documents generated by the QA planner agent (per ADR-0030). Use alongside the implementation plan template.
...
<!-- Flat table of all findings. Use the Classification column to distinguish
     quality concerns from preferences. Do NOT split into separate subsections. -->

| # | Stage | Classification | Finding | Recommendation | Plan Ref | Status |
|---|-------|----------------|---------|----------------|----------|--------|
| 1 | 1 | Quality concern | Description | Recommendation | Task 1.1 criteria 3 | ⚠️ Open / ✅ Resolved / Won't Fix |
| 2 | 1 | Preference | Description | Reason for deferral | Task 1.2 | — |

<!-- Won't Fix rationale: when a quality concern will not be remediated,
     set its Status to "Won't Fix" and append a brief rationale below the
     table under a "### Won't Fix — rationale" heading. If no quality
     concerns are being skipped, omit the heading. -->
```
...
| **Recommendations** | Flat table of all findings with Classification column (quality concern / preference) and Plan Ref column (links to specific plan task or acceptance criterion the finding supplements). Won't Fix rationale follows the table when applicable |
