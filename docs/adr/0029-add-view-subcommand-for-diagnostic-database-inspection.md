...
**Validation needs:** None — all options use well-understood patterns (TSV output, JSONL serialization, Diesel queries). Option 4 was selected after evaluating all four options against the five decision drivers; it is the only option that scores positively on all five (Unix composability, plumbing identity, discoverability, round-trip symmetry, minimal complexity).
...
**Field-name alignment:** JSONL output uses ingest-compatible field names (`commit`, not `commit_sha`) so that `adr-db view task_summaries --output jsonl | adr-db ingest` round-trips correctly. The TSV header uses DB column names (`commit_sha`) since TSV is for human inspection, not re-ingestion.
...
- Risk of proto-porcelain calcification — if the downstream porcelain project is delayed indefinitely, `view` may become the de facto inspection interface, accumulating user expectations and informal dependencies that the instability contract cannot fully prevent.
...
### Q: Is the Conclusion Checkpoint duplicated?

**Addressed** — Removed the blank duplicate checkpoint block that appeared after the completed one. Template duplication artifact.

### Q: Does the JSONL round-trip claim account for field-name asymmetry between ingest and the database schema?

**Addressed** — Added explicit "Field-name alignment" note in the Output Formats section clarifying that JSONL output uses ingest-compatible field names (`commit`, not `commit_sha`) for round-trip, while TSV uses DB column names for human inspection.

### Q: Does the evaluator agent claim in the Evaluation Checkpoint provide sufficient evidence?

**Addressed** — Replaced "An independent evaluator agent confirmed…" with a grounded statement describing the actual evaluation: Option 4 is the only option that scores positively on all five decision drivers.

### Q: Should the ADR specify a default for `--limit`?

**Deferred** — The concern is valid but out of scope for this ADR. The `--limit` default is an implementation detail; the instability contract makes any stated default non-binding. → Follow-up: implementation

### Q: Are the negative consequences understated by inline mitigations?

**Addressed** — Added an unmitigated negative consequence: proto-porcelain calcification risk — if the downstream porcelain project is delayed indefinitely, `view` may accumulate user expectations the instability contract cannot fully prevent.
