...
**Framing:**
The wi-nygard-agent format needs a status beyond Accepted to indicate that a decision has been fully delivered (implemented, tested, deployed). A Deliverables section makes scope visible at a glance and enables work-item-driven lifecycle orchestration.

**Tolerance:**
- Risk: Low — extends an existing template with additive changes
- Change: Medium — new status and section in the template format
- Improvisation: Low — the Delivered status and Deliverables section are tightly specified

**Uncertainty:**
- Certain: teams need to distinguish "decided" from "delivered"; the wi-nygard-agent template needs extension
- Uncertain: whether Deliverables should be mandatory or optional; exact status lifecycle transitions
...
**Candidates:**
- Add Delivered status + Deliverables section to wi-nygard-agent template
- Use existing Accepted status with a metadata tag for delivery tracking
- Track delivery outside the ADR (in work item system only)
