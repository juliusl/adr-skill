...
- Vendor-specific webhook setup required — GitHub and ADO have different event systems
...
Extend the format script with a `lifecycle` subcommand that serves as the entry point for state-driven actions. The developer invokes `lifecycle` with a vendor and ID; the orchestrator:
...
The mapping uses normalized work item states from ADR-0035. Vendor-specific states (e.g., ADO's "New" vs GitHub's "open") are already normalized by the adapter layer.
...
**Interface:**
```
wi-nygard-agent-format.sh lifecycle <vendor> <id> [--auto] [--sync]
```
...
```json
{"vendor":"gh","id":"42","from_status":"Proposed","to_status":"Accepted","action":"implement-adr","trigger":"lifecycle --auto","timestamp":"2026-04-05T08:00:00Z"}
```
...
**Negative:**
- The lifecycle command must still be invoked — it doesn't watch for changes. Teams wanting real-time automation must wire it into CI/CD triggers, which requires per-vendor setup (GitHub Actions vs ADO Pipelines).
...
- Event-driven webhook integration per vendor
